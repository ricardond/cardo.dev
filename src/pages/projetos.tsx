import Card from "@/components/Card";

export default function Projetos(){
    return(
        <section className="flex flex-col justify-center items-center flex-wrap gap-5 mt-20">
            <h1 className="text-3xl text-amber-300 font-bold">Meus Projetos</h1>
            <div className="flex flex-wrap gap-5 w-full justify-center items-center">
                <Card titulo="Onepedia" desc="Site sobre assuntos relacionados a Onepiece." image="https://onepedia.netlify.app/img/background.jpg"/>
                <Card titulo="Game: Cubic War" desc="Jogo criado com uso da Godot Engine." image="https://img.itch.zone/aW1nLzE4NjYwODUzLnBuZw==/original/BVOvET.png"/>
                <Card titulo="Game: Zoro Run" desc="Jogo com temática do Zoro, criado com canvas + Javascript." image="https://private-user-images.githubusercontent.com/96152954/285897041-56bf2ad8-e56e-4f3e-a9ee-c582f317b9bb.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..7Th8FsA6PXNWzEoDvS-LwjwkomAdW8aUKO7c-hKvR24"/>
            </div>
        </section>
    )
}